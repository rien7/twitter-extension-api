import { afterEach, describe, expect, it, vi } from 'vitest';

import { deleteTweet } from '../api/action/delete-tweet';
import type { DeleteTweetOriginalResponse } from '../api/action/delete-tweet/types';

describe('deleteTweet response normalization', () => {
  afterEach(() => {
    vi.restoreAllMocks();
    vi.unstubAllGlobals();
  });

  it('lifts common fields and keeps full raw payload in __original', async () => {
    const rawPayload: DeleteTweetOriginalResponse = {
      data: {
        delete_tweet: {
          tweet_results: {
            result: {
              rest_id: '42'
            }
          }
        }
      }
    };

    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
        return new Response(JSON.stringify(rawPayload), {
          status: 200,
          headers: { 'content-type': 'application/json' }
        });
      })
    );

    const response = await deleteTweet({
      tweetId: '42'
    });

    expect(response.success).toBe(true);
    expect(response.tweetId).toBe('42');
    expect(response.deletedTweetId).toBe('42');
    expect(response.__original).toEqual(rawPayload);
  });
});
