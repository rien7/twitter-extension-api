...
## Purpose
Fetch a user's tweets timeline from Twitter/X GraphQL.

This API is suitable for extensions that need:
- profile tweet tab rendering,
- cursor-based pagination for user tweets,
- lightweight tweet/user metadata extraction.

## Endpoint
- Method: `GET`
- Path pattern: `/i/api/graphql/*/UserTweets`
...
## Request Type
Type name: `UserTweetsRequest`

### Core fields
- `userId`: Optional target user id for the tweets timeline (defaults to self user id from `twid` cookie).
- `count`: Convenience override for `variables.count`.
- `cursor`: Pagination cursor for next-page requests.
- `withQuickPromoteEligibilityTweetFields`: Convenience override for quick-promote fields.
...
### Default behavior
`buildUserTweetsRequest(input)` merges in this order:
